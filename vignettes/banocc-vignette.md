...
The initial values are sampled from the priors by default because the positive definite constraint on the covariance matrix makes it difficult for `rstan::sampling` to generate initial values automatically. They can also be set to a particular value by using a list.
